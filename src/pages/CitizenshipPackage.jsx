...
const CitizenshipPackage = () => {
  ...
  const getPackageData = async () => {
    try {
      ...
      const docRef = doc(db, 'citizenships', params?.id);
          const snapshot = await getDoc(docRef);
          if (snapshot.exists()) {
            setLoading(false);
             setPackageData({
          packageName: snapshot.data()?.name,
          packageDescription: snapshot.data()?.description,
          packageDays: 5,
          packageNights: 6,
          packageAccommodation: 9,
          packagePrice: 500,
          packageImage: `https://firebasestorage.googleapis.com/v0/b/gooutfilter.appspot.com/o/citizenship%2F${snapshot.data().name}.jpg?alt=media&token=bfa49e65-3d50-4117-8f74-1965c38854b8`,
        });
          } else {
            setError(snapshot?.message || "Something went wrong!");
            setLoading(false);
          }
    } catch (error) {
      //console.log(error);
...
export default CitizenshipPackage;
